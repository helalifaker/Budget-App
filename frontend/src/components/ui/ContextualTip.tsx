...
/**
 * Hook to manage contextual tips state
 * Useful when you need to programmatically control tips visibility
 */
export function useContextualTips(storageKeyPrefix = 'tip-dismissed') {
  const [isGloballyDisabled, setIsGloballyDisabled] = useState(false)

  useEffect(() => {
    const globalKey = `${storageKeyPrefix}-global`
    setIsGloballyDisabled(localStorage.getItem(globalKey) === 'true')
  }, [storageKeyPrefix])

  const enableAllTips = useCallback(() => {
    const globalKey = `${storageKeyPrefix}-global`
