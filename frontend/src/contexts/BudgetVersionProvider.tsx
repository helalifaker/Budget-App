 ...
 *
 * Provides global budget version selection state to the entire application.
...
/** LocalStorage key for persisting selected budget version */
const STORAGE_KEY = 'efir-selected-budget-version'
...
function persistVersionId(id: string | undefined): void {
  try {
    ...
    console.warn('[BudgetVersionProvider] Failed to persist version ID')
  }
}
...
export function BudgetVersionProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (defaultVersion) {
      console.log(
        '[BudgetVersionProvider] Auto-selecting version:',
        defaultVersion.name,
  ...
  return <BudgetVersionContext.Provider value={value}>{children}</BudgetVersionContext.Provider>
}
