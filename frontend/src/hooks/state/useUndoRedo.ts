 ...
 * @example
 * ```typescript
 * const { undo, redo, canUndo, canRedo, undoStack, redoStack } = useUndoRedo(versionId);
 *
 * // Undo last session
 * await undo();
 *
 ...
 * ```
 */
export function useUndoRedo(versionId: string, moduleCode?: string) {
  ...
  const { data: changes } = useQuery({
    queryKey: ['cell-changes', versionId, moduleCode],
    queryFn: async () => {
