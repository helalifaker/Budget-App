 ...
 * @param options.versionId - Current version to subscribe to
...
export function useRealtimeSync(options: RealtimeSyncOptions) {
  const { versionId, onCellChanged, onConnectionChange, showNotifications = true } = options
  ...
  const handleChange = useCallback(
    (payload: RealtimePayload<CellData>) => {
      ...
      if (eventType === 'UPDATE') {
        ...
        queryClient.setQueryData<CellData[]>(['cells', versionId], (oldData) => {
    ...
    [versionId, user?.id, queryClient, onCellChanged, showNotifications]
  )
  ...
  useEffect(() => {
    ...
    // Create channel for this version
    const realtimeChannel = supabase.channel(`budget:${versionId}`, {
      config: {
  ...
  }, [versionId, user, handleChange, updateStatus, showNotifications])
