...
import { useConsolidationSummary } from '@/hooks/api/useConsolidation'
...
export function useSidebarStats(): SidebarStats {
  ...
  } = useConsolidationSummary(selectedVersionId)
  ...
  const stats = useMemo<SidebarStats>(() => {
    ...
    if (consolidationData) {
      ...
      const totalExpenses = consolidationData.total_expenses ?? null
