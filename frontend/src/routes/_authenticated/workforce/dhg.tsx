/**
 * DHG Workforce Planning Page - /workforce/dhg
 *
...
export const Route = createFileRoute('/_authenticated/workforce/dhg')({
  component: DHGPage,
})
...
function SubjectHoursTab({
  ...
  isLoading,
  onCellValueChanged,
}: { data: SubjectHoursItem[]; isLoading: boolean } & TabProps<SubjectHoursItem>) {
  const columnDefs: ColumnDef<SubjectHoursItem, unknown>[] = useMemo(
    () => [
      {
      ...
      },
      {
        ...
        accessorFn: (row) => {
          ...
          const classes = row.number_of_classes || 1
          return hours * classes
        },
        cell: ({ getValue }) => {
          const value = getValue() as number
          return <span className="font-semibold text-slate-700">{value?.toFixed(1) ?? '0.0'}</span>
        },
        meta: {
          editable: false,
          align: 'right',
  ...
  return (
    <Card>
      ...
      <CardContent>
        <EditableTable<SubjectHoursItem>
          rowData={data}
          columnDefs={columnDefs}
          getRowId={(row) => row.id || `${row.level_id}-${row.subject_id}`}
...
function TeacherFTETab({
  ...
  isLoading,
  onCellValueChanged,
}: { data: TeacherFTEItem[]; isLoading: boolean } & TabProps<TeacherFTEItem>) {
  const columnDefs: ColumnDef<TeacherFTEItem, unknown>[] = useMemo(
    () => [
      {
      ...
      {
        accessorKey: 'fte_required',
        header: 'FTE Required',
        size: 140,
        cell: ({ getValue }) => {
          const value = getValue() as number
          return <span className="font-bold">{value?.toFixed(2) ?? '0.00'}</span>
        },
        meta: {
          editable: false,
          align: 'right',
...
function TRMDTab({
  ...
  isLoading,
  onCellValueChanged,
}: { data: TRMDItem[]; isLoading: boolean } & TabProps<TRMDItem>) {
  const columnDefs: ColumnDef<TRMDItem, unknown>[] = useMemo(
    () => [
      {
        accessorKey: 'subject_id',
        header: 'Subject',
      ...
      },
      {
        ...
        cell: ({ getValue }) => {
          ...
          const icon = value > 0 ? '▼' : '▲'
          const colorClass = value > 0 ? 'text-terracotta' : 'text-sage'
          return (
            <span className={cn('font-bold', colorClass)}>
              {icon} {Math.abs(value).toFixed(2)} FTE
            </span>
          )
        },
        meta: {
  ...
  return (
    <Card>
      ...
      <CardContent>
        <EditableTable<TRMDItem>
          rowData={data}
          columnDefs={columnDefs}
          getRowId={(row) => `${row.subject_id}-${row.cycle_id}`}
          onCellValueChanged={onCellValueChanged}
          loading={isLoading}
          height={600}
        />
...
// Helper to compute HSA status
const getHSAStatus = (
  hours: number
): { label: string; variant: 'destructive' | 'success' | 'warning' } => {
  if (hours > 4) return { label: 'Exceeds Max', variant: 'destructive' }
  if (hours >= 2) return { label: 'Within Limit', variant: 'success' }
  return { label: 'Below Target', variant: 'warning' }
}
...
function HSATab({
  ...
  isLoading,
  onCellValueChanged,
}: { data: HSAItem[]; isLoading: boolean } & TabProps<HSAItem>) {
  // Wrap onCellValueChanged to add validation
  const handleCellValueChanged = useCallback(
    (event: CellValueChangedEvent<HSAItem>) => {
      // Track as draft change
      onCellValueChanged?.(event)
      // Validation warning
      if (event.field === 'hsa_hours') {
        const hours = event.newValue as number
        if (hours > 4) {
  ...
  const columnDefs: ColumnDef<HSAItem, unknown>[] = useMemo(
    () => [
      {
      ...
      },
      {
        ...
        header: 'Status',
        size: 140,
        accessorFn: (row) => row.hsa_hours,
        cell: ({ getValue }) => {
          const hours = (getValue() as number) || 0
          const status = getHSAStatus(hours)
  ...
  return (
    <Card>
      ...
      <CardContent>
        <EditableTable<HSAItem>
          rowData={data}
          columnDefs={columnDefs}
          getRowId={(row) => row.id || `${row.teacher_name}-${row.subject_name}`}
          onCellValueChanged={handleCellValueChanged}
          loading={isLoading}
          height={600}
        />
