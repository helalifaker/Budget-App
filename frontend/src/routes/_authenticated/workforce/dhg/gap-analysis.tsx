 ...
 *
 * @module /workforce/dhg/gap-analysis
 */
...
function GapAnalysisPage() {
  ...
  // Column definitions
  const columnDefs = useMemo<ColDef<TRMDGapItem>[]>(() => {
    // Reusable cell styles - defined inside useMemo to avoid recreating on every render
    const rightAlignStyle: CellStyle = { textAlign: 'right' }
    const rightAlignBoldStyle: CellStyle = { textAlign: 'right', fontWeight: '500' }
    ...
    return [
      ...
      {
        headerName: 'Moyens (FTE)',
        field: 'total_allocated',
        width: 120,
        type: 'numericColumn',
        valueFormatter: (params: ValueFormatterParams) => params.value?.toFixed(2) || '0.00',
        cellStyle: rightAlignBoldStyle,
      },
      {
        headerName: 'Gap',
        field: 'deficit',
        width: 100,
        type: 'numericColumn',
        cellRenderer: (params: ICellRendererParams<TRMDGapItem>) => {
          const deficit = params.value as number
          if (deficit > 0) {
            return <span className="text-red-600 font-medium">-{deficit.toFixed(2)}</span>
          } else if (deficit < 0) {
            return (
              <span className="text-green-600 font-medium">+{Math.abs(deficit).toFixed(2)}</span>
            )
          }
          ...
          <CardContent className="flex items-center gap-4 py-6">
            ...
            <div>
              ...
              <p className="text-sm text-amber-700">
                Please select a budget version from the header to view gap analysis.
              </p>
            </div>
          </CardContent>
        </Card>
