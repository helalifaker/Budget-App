...
- UNIQUE (version_id, cycle_id, level_id, effective_date) - prevents duplicates
...
**Indexes:**
- Primary key on id
- Index on (version_id, cycle_id) for cycle-level queries
- Index on (version_id, level_id) for level lookups
...
const maternelleParams = {
  version_id: testBudgetVersion.id,
...
**Test Data:**
```typescript
const version2025 = { version_id: version2025.id, target_class_size: 24, effective_date: "2025-09-01" };
const version2026 = { version_id: version2026.id, target_class_size: 26, effective_date: "2026-09-01" };
```
