...
**Constraints:**
- Unique constraint on (version_id, kpi_definition_id) - one value per KPI per version
...
**Indexes:**
- Primary key on id
- Index on (version_id, kpi_definition_id) for lookups
...
# Store in kpi_values
{
  ...
  "calculation_inputs": {
    ...
    "enrollment_query": "SELECT SUM(student_count) FROM students_data WHERE version_id = ?",
    "fte_query": "SELECT SUM(fte_required) FROM teachers_dhg_requirements WHERE version_id = ?"
  }
}
```
...
# Store in kpi_values
{
  ...
  "calculation_inputs": {
    ...
    "revenue_query": "SELECT SUM(amount_sar) FROM finance_data WHERE version_id = ? AND data_type = 'revenue'",
    "enrollment_query": "SELECT SUM(student_count) FROM students_data WHERE version_id = ?"
  }
}
```
...
# Store in kpi_values
{
  ...
  "calculation_inputs": {
    ...
    "dhg_query": "SELECT SUM(hours_required) FROM teachers_dhg_subject_hours WHERE version_id = ? AND level_id IN (...)",
    "enrollment_query": "SELECT SUM(student_count) FROM students_data WHERE version_id = ? AND level_id IN (...)"
  }
}
```
...
1. **settings_versions**: Parent table for version control
...
- **Historical Tracking**: By linking to settings_versions, the system maintains a complete history of KPI values for trend analysis
