...
### Schema Naming Note (Phase 3+)

This document predates the schema standardization. When implementing these enhancements:
- Use `version_id` (not `budget_version_id`)
- Use `settings_versions` (not `budget_versions`)
- Use prefixed table names under `efir_budget` (e.g., `settings_*`, `students_*`, `ref_*`)
...
```
GET /api/v1/class-distribution/{version_id}
...
```typescript
export function useClassDistribution(versionId: string) {
  return useQuery({
    queryKey: ['class-distribution', versionId],
    queryFn: () => api.getClassDistribution(versionId),
...
CREATE TABLE class_size_scenarios (
    ...
    CONSTRAINT uk_scenario_name UNIQUE(version_id, scenario_name)
);
...
-- Indexes
CREATE INDEX idx_scenarios_version ON class_size_scenarios(version_id);
...
-- Policy: Users can view scenarios in their organization's versions
CREATE POLICY scenarios_select_policy ON class_size_scenarios
    FOR SELECT USING (
        version_id IN (
            SELECT id FROM settings_versions
...
```typescript
export function useScenarios(versionId: string) {
  return useQuery({
    queryKey: ['scenarios', versionId],
    queryFn: () => api.getScenarios(versionId),
    staleTime: 2 * 60 * 1000,
  })
}
...
CREATE INDEX idx_optimization_recommendations_version
    ON optimization_recommendations(version_id);
...
@router.get("/optimization-history/{version_id}", response_model=List[OptimizationHistoryItem])
async def get_optimization_history(
    version_id: str,
    ...
    """
    Get optimization history for a version.
...
export function useOptimizationHistory(versionId: string) {
  return useQuery({
    queryKey: ['optimization-history', versionId],
    queryFn: () => api.getOptimizationHistory(versionId),
    staleTime: 5 * 60 * 1000,
  })
}
```
...
```python
async def invalidate_class_size_calculations(self, version_id: str):
    """Invalidate cached class size calculations."""
    await self.redis.delete(f"class_distribution:{version_id}")
    await self.redis.delete(f"scenarios:{version_id}")
    await self.redis.delete(f"optimization:{version_id}")
```
...
export function useRealtimeClassSizes(versionId: string) {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`class-sizes:${versionId}`, {
      config: {
    ...
    channel
      ...
      .on('postgres_changes', {
        ...
        queryClient.invalidateQueries(['class-distribution', versionId])
        queryClient.invalidateQueries(['scenarios', versionId])
        queryClient.invalidateQueries(['optimization', versionId])
  ...
  }, [versionId, user, queryClient])

  const reconnect = () => {
    setConnectionStatus('CONNECTING')
    // Force reconnection by unmounting and remounting
    queryClient.invalidateQueries(['class-size-params', versionId])
  }
...
export function PresenceIndicator({ versionId }: Props) {
  const { activeUsers, isLoading } = useUserPresence({ versionId })
...
// Inside component
const { connectionStatus, reconnect } = useRealtimeClassSizes(versionId)

// Add to header
<div className="flex items-center gap-4">
  <PresenceIndicator versionId={versionId} />
