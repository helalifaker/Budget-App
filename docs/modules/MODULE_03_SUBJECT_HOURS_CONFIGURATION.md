...
#### 2. settings_subject_hours_matrix
...
subject_id            UUID NOT NULL FOREIGN KEY -> ref_subjects.id (RESTRICT)
...
**Constraints:**
- UNIQUE (version_id, subject_id, level_id) - one hours value per subject per level per version
...
**Indexes:**
- Primary key on id
- Unique index on (version_id, subject_id, level_id)
...
1. **Model Tests**: Verify ref_subjects and settings_subject_hours_matrix models
