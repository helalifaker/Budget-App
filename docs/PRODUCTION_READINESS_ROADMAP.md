...
ON enrollment (budget_version_id, level_id)
INCLUDE (student_count, nationality);

CREATE INDEX CONCURRENTLY idx_dhg_budget_subject_covering
ON dhg_hours (budget_version_id, subject_id, level_id)
INCLUDE (hours_per_week, num_classes);
```
