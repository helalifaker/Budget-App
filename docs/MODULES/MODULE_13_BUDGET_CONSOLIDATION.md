...
superseded_by_id      UUID NULL FOREIGN KEY -> budget_versions.id (SET NULL)
notes                 TEXT NULL
...
```sql
id                    UUID PRIMARY KEY
budget_version_id     UUID NOT NULL UNIQUE FOREIGN KEY -> budget_versions.id (CASCADE)
...
**RLS Policies:**
- Same as budget_versions

**Indexes:**
- Primary key on id
- Index on budget_version_id (unique constraint)
...
- UNIQUE (budget_version_id, account_code, cost_center_id)
...
**Indexes:**
- Primary key on id
- Index on (budget_version_id, line_item_type)
...
**Indexes:**
- Primary key on id
- Index on (budget_version_id, performed_at DESC) for audit trail
...
POST   /api/v1/compare-budgets
       Request: { budget_version_ids: [id1, id2, id3] }
