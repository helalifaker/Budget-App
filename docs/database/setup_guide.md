...
-- Note: Table count evolves over time. After Phase 3B+ you should see
-- prefixed tables like:
--   ref_academic_cycles, ref_academic_levels, ref_subjects, ...
--   settings_versions, settings_system_configs, ...
--   students_*, teachers_*, finance_*, insights_*, admin_*

-- Check alembic version
SELECT version_num FROM efir_budget.alembic_version;
-- Expected: latest migration revision (head)
```
...
RLS policies are applied via Alembic migrations (`backend/alembic/versions/`). Do not manually execute a standalone
RLS script unless you are intentionally repairing a broken environment.
