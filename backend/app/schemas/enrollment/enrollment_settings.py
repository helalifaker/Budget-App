...
class CalibrationRequest(BaseModel):
    ...
    The version_id is required to determine the TARGET year for calibration.
    For example, Budget 2026 targets academic year 2026-2027, so calibration uses
    historical data from N-1 (2025-2026) and N-2 (2024-2025).
    """

    version_id: UUID = Field(
        ...,
        description="Budget version ID - determines the target academic year for calibration",
    )
    force: bool = Field(
        default=False,
...
class CalibrationResult(BaseModel):
    ...
    source_years: list[str] = Field(default_factory=list)
    target_school_year: str | None = Field(
        default=None,
        description="Target academic year for which rates were calibrated (e.g., '2026/2027')",
    )
