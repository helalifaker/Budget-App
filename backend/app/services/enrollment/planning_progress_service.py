...
from app.services.enrollment.enrollment_capacity import get_effective_capacity
...
class PlanningProgressService:
    ...
    async def get_planning_progress(
        ...
        enrollment_progress = await self._validate_enrollment_step(version_id)
        class_structure_progress = await self._validate_class_structure_step(
            version_id, enrollment_progress
        )
        dhg_progress = await self._validate_dhg_step(
            version_id, class_structure_progress
        )
        revenue_progress = await self._validate_revenue_step(
            version_id, enrollment_progress
        )
        costs_progress = await self._validate_costs_step(
            version_id, dhg_progress
        )
        capex_progress = await self._validate_capex_step(version_id)
    ...
    async def _get_version(self, version_id: UUID) -> BudgetVersion:
        """Get version or raise NotFoundError."""
    ...
    async def _validate_enrollment_step(
        ...
        result = await self.db.execute(
            ...
            ).where(EnrollmentPlan.version_id == version_id)
        )
        enrollment_count, total_students = result.one()
        total_students = int(total_students or 0)
        effective_capacity = await get_effective_capacity(self.db, version_id)
    ...
    async def _validate_class_structure_step(
        self, version_id: UUID, enrollment_progress: StepProgress
    ) -> StepProgress:
        """
        ...
        result = await self.db.execute(
            ...
            ).where(ClassStructure.version_id == version_id)
        )
    ...
    async def _validate_dhg_step(
        self, version_id: UUID, class_structure_progress: StepProgress
    ) -> StepProgress:
        """
        ...
        subject_hours_result = await self.db.execute(
            select(func.count(DHGSubjectHours.id)).where(
                DHGSubjectHours.version_id == version_id
            )
        )
        ...
        fte_result = await self.db.execute(
            ...
            ).where(DHGTeacherRequirement.version_id == version_id)
        )
    ...
    async def _validate_costs_step(
        self, version_id: UUID, dhg_progress: StepProgress
    ) -> StepProgress:
        """
    ...
    async def _validate_capex_step(self, version_id: UUID) -> StepProgress:
        """
