...
class ClassStructureService:
    ...
    async def get_class_structure(
        ...
        Performance Notes:
            ...
            - Eager loads level, cycle, version, and audit fields
    ...
    async def calculate_class_structure(
        ...
        if not enrollments:
            raise BusinessRuleError(
                "NO_ENROLLMENT_DATA",
                "Données d'inscription manquantes. Veuillez d'abord saisir les effectifs dans la page Planification.",
            )
        ...
        if not class_size_params:
            raise BusinessRuleError(
                ...
                "Paramètres de taille de classe manquants. Veuillez d'abord configurer les paramètres dans Configuration > Class Sizes.",
            )
        ...
        for level_id, (total_students, level, cycle) in enrollments.items():
            ...
            if level_id in existing_by_level:
                ...
                if user_id and not str(user_id).startswith("00000000-0000-0000-0000-"):
                    existing.updated_by_id = user_id
                    
                results.append(existing)
            else:
                # Create new
                new_obj = ClassStructure(**data)
                
                if user_id and not str(user_id).startswith("00000000-0000-0000-0000-"):
                    new_obj.created_by_id = user_id
                    new_obj.updated_by_id = user_id
    ...
    async def bulk_update_class_structure(
        ...
        Args:
            updates: List of update dictionaries containing 'id' and fields to update
            user_id: User ID for audit trail

        Returns:
            List of updated ClassStructure instances
        """
        if not updates:
            return []
        ...
        query = select(ClassStructure).where(ClassStructure.id.in_(ids))
        result = await self.session.execute(query)
        existing_records = result.scalars().all()
        existing_map = {cs.id: cs for cs in existing_records}
        
        updated_instances = []
        
        for update_data in updates:
            cs_id = update_data["id"]
            if cs_id not in existing_map:
                continue
                
            class_structure = existing_map[cs_id]
            fields_to_update = update_data.copy()
            fields_to_update.pop("id")
            
            # Extract fields for validation/logic
            total_students = fields_to_update.get("total_students")
            number_of_classes = fields_to_update.get("number_of_classes")
            avg_class_size = fields_to_update.get("avg_class_size")
            atsem_count = fields_to_update.get("atsem_count")

            # Validation
            if number_of_classes is not None and number_of_classes <= 0:
                raise ValidationError("Number of classes must be greater than 0", field="number_of_classes")
                
            if avg_class_size is not None and (avg_class_size <= 0 or avg_class_size > 50):
                raise ValidationError("Average class size must be between 0 and 50", field="avg_class_size")

            if atsem_count is not None and atsem_count < 0:
                 raise ValidationError("ATSEM count must be non-negative", field="atsem_count")

            # Logic for average class size recalculation
            recalc_avg = False
            if (total_students is not None and number_of_classes is None) or \
               (number_of_classes is not None and total_students is None):
                recalc_avg = True
                
            if recalc_avg:
                new_students = fields_to_update.get("total_students", class_structure.total_students)
                new_classes = fields_to_update.get("number_of_classes", class_structure.number_of_classes)
                if new_classes > 0:
                     fields_to_update["avg_class_size"] = Decimal(new_students / new_classes).quantize(Decimal("0.01"))

            # Apply updates
            for field, value in fields_to_update.items():
                if hasattr(class_structure, field) and value is not None:
                    setattr(class_structure, field, value)
            
            # Audit fields
            class_structure.updated_at = datetime.utcnow()
            if user_id and not str(user_id).startswith("00000000-0000-0000-0000-"):
                class_structure.updated_by_id = user_id

            updated_instances.append(class_structure)
