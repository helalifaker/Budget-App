...
class KPIService:
    ...
    async def calculate_kpis(
        ...
        Args:
            version_id: Version UUID (used as cache key)
        ...
        kpi_data = await self._get_kpi_calculation_data(version_id)
    ...
    async def save_kpi_values(
        ...
        for kpi_code, result in kpi_results.items():
            ...
            query = select(KPIValue).where(
                and_(
                    KPIValue.version_id == version_id,
    ...
    async def get_kpi_by_type(
        ...
        Performance Notes:
            ...
            - Eager loads kpi_definition, version, and audit fields
    ...
    async def get_benchmark_comparison(
        ...
        kpi_values = await self.get_all_kpis(version_id)
    ...
    async def _get_kpi_calculation_data(
        ...
        total_students_query = select(func.coalesce(func.sum(EnrollmentPlan.student_count), 0)).where(
            and_(EnrollmentPlan.version_id == version_id, EnrollmentPlan.deleted_at.is_(None))
        )
        ...
        total_classes_query = select(func.coalesce(func.sum(ClassStructure.number_of_classes), 0)).where(
            and_(ClassStructure.version_id == version_id, ClassStructure.deleted_at.is_(None))
        )
        ...
        total_hours_query = select(func.coalesce(func.sum(DHGSubjectHours.total_hours_per_week), 0)).where(
            and_(DHGSubjectHours.version_id == version_id, DHGSubjectHours.deleted_at.is_(None))
        )
        ...
        teacher_fte_query = select(func.coalesce(func.sum(DHGTeacherRequirement.simple_fte), 0)).where(
            and_(
                DHGTeacherRequirement.version_id == version_id,
