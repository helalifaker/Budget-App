...
class BudgetActualService:
    ...
    async def import_actuals(
        ...
        Args:
            version_id: Budget version UUID to compare against
        ...
        try:
            ...
            for period in periods_covered:
                try:
                    await self.calculate_variance(version_id, period)
    ...
    async def import_actuals_from_file(
        self,
        version_id: uuid.UUID,
        ...
        """
        Import actual financial data from parsed file records.

        Args:
            version_id: Budget version UUID to compare against
            records: List of parsed records with:
                - account_code: French PCG account code
                - description: Account description (optional)
                - amount_sar: Amount in SAR
                - period: Period string (T1, T2, T3, ANNUAL)
            period: Period string (T1, T2, T3, ANNUAL)
        ...
        try:
            ...
            # Map period string to month number for variance calculation
            period_to_month = {
                "T1": 4,  # End of T1 (Sep-Dec) ~ month 4
                "T2": 8,  # End of T2 (Jan-Apr) ~ month 8
                "T3": 12,  # End of T3 (May-Aug) ~ month 12
                "ANNUAL": 12,  # Full year
            }
            month_for_variance = period_to_month.get(period, 12)

            # Get fiscal year from budget version
            fiscal_year = version.academic_year  # e.g., "2024-2025" -> extract start year
            ...
            for record in records:
                ...
                # Create ActualData record
                actual_data = ActualData(
                    fiscal_year=fiscal_year if isinstance(fiscal_year, int) else 2025,
                    period=month_for_variance,
                    account_code=account_code,
                    account_name=record.get("description"),
                    amount_sar=Decimal(str(record.get("amount_sar", 0))),
                    currency="SAR",
                    import_batch_id=import_batch_id,
                    import_date=datetime.utcnow(),
            ...
            # Calculate variance for the period
            try:
                await self.calculate_variance(version_id, month_for_variance)
            except Exception as exc:
                logger.warning(
                    "Failed to calculate variance after file import",
                    version_id=str(version_id),
                    period=period,
                    error=str(exc),
                )
            ...
            logger.error(
                "Failed to import actuals from file",
                version_id=str(version_id),
                period=period,
            ...
            raise ServiceException(
                "Failed to import actuals from file. Please try again.",
                status_code=500,
                details={"version_id": str(version_id), "period": period},
            ) from e
    ...
    async def calculate_variance(
        ...
        budget_amounts = await self._get_budget_amounts(version_id, account_code)
        ...
        for actual in actuals:
            ...
            existing_query = select(BudgetVsActual).where(
                and_(
                    BudgetVsActual.version_id == version_id,
    ...
    async def _get_budget_amounts(
        ...
        Args:
            version_id: Budget version to pull consolidation for
