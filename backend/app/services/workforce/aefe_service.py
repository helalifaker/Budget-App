...
class AEFEService(BaseService[AEFEPosition]):
    ...
    async def initialize_positions(
        ...
        existing = await self.get_positions_by_version(version_id)
        if existing:
            raise ValidationError(
                f"AEFE positions already exist for budget version {version_id}. "
    ...
    async def assign_employee(
        ...
        existing_query = (
            ...
            .where(AEFEPosition.version_id == position.version_id)
            .where(AEFEPosition.deleted_at.is_(None))
        )
    ...
    async def get_position_summary(
        ...
        """
        positions = await self.get_positions_by_version(version_id)
