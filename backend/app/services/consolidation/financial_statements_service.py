...
class FinancialStatementsService:
    ...
    async def get_balance_sheet(
        ...
        return await self._generate_balance_sheet(version_id)
    ...
    async def calculate_statement_lines(
        ...
        if statement_type == StatementType.INCOME_STATEMENT:
            ...
            return await self._calculate_balance_sheet_assets_lines(version_id)
        elif statement_type == StatementType.BALANCE_SHEET_LIABILITIES:
            return await self._calculate_balance_sheet_liabilities_lines(version_id)
        elif statement_type == StatementType.CASH_FLOW:
            return await self._calculate_cash_flow_lines(version_id)
        else:
    ...
    async def _get_existing_statement(
        ...
        query = (
            ...
            .where(
                and_(
                    FinancialStatement.version_id == version_id,
    ...
    async def _generate_income_statement(
        ...
        # Create statement
        statement_name = (
            f"Compte de résultat {version.academic_year}"
            if statement_format == StatementFormat.FRENCH_PCG
            else f"Income Statement {version.academic_year}"
        )
    ...
    async def _generate_balance_sheet(
        ...
        # Generate assets
        assets_lines = await self._calculate_balance_sheet_assets_lines(version_id)
        ...
        assets_statement = await self.statement_service.create({
            ...
            "statement_name": f"Bilan - Actif {version.academic_year}",
            "fiscal_year": version.fiscal_year,
        ...
        liabilities_statement = await self.statement_service.create({
            ...
            "statement_format": StatementFormat.FRENCH_PCG,
            "statement_name": f"Bilan - Passif {version.academic_year}",
            "fiscal_year": version.fiscal_year,
    ...
    async def _calculate_balance_sheet_assets_lines(
        ...
        query = (
            ...
            .where(
                and_(
                    BudgetConsolidation.version_id == version_id,
                    BudgetConsolidation.source_table == "finance_capex_plans",
                    BudgetConsolidation.deleted_at.is_(None),
                )
            )
        )
