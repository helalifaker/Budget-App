...
class WritebackService:
    ...
    async def get_cell_by_id(
        ...
        query = """
            SELECT
                id, version_id, module_code, entity_id, field_name,
                ...
                (SELECT COUNT(*) FROM efir_budget.admin_cell_comments cc WHERE cc.cell_id = pc.id) as comment_count,
                (SELECT COUNT(*) FROM efir_budget.admin_cell_comments cc WHERE cc.cell_id = pc.id AND NOT cc.is_resolved) as unresolved_comment_count
            FROM efir_budget.admin_planning_cells pc
    ...
    async def create_cell(
        ...
        query = """
            INSERT INTO efir_budget.admin_planning_cells (
                id, version_id, module_code, entity_id, field_name,
                period_code, value_numeric, value_text, value_type,
            ...
            RETURNING id, version_id, module_code, entity_id, field_name,
                      period_code, value_numeric, value_text, value_type,
                      is_locked, lock_reason, locked_by, locked_at,
                      version, modified_by, modified_at, created_by_id, created_at
        """
        ...
        # Invalidate caches
        await self._invalidate_module_cache(data.module_code, str(data.version_id))
    ...
    async def update_cell(
        ...
        # Perform update
        query = """
            UPDATE efir_budget.admin_planning_cells
            SET
        ...
        # Invalidate caches
        await self._invalidate_module_cache(cell["module_code"], str(cell["version_id"]))
    ...
    async def batch_update_cells(
        ...
        affected_modules: set[tuple[str, str]] = set()  # (module_code, version_id)
        ...
        for idx, cell_update in enumerate(batch.updates):
            try:
                ...
                if row is not None:
                    ...
                    affected_modules.add((cell["module_code"], str(cell["version_id"])))
        ...
        for module_code, version_id in affected_modules:
            await self._invalidate_module_cache(module_code, version_id)
    ...
    async def get_change_history(
        ...
        query = """
            SELECT
                id, cell_id, version_id, module_code, entity_id, field_name,
            ...
            FROM efir_budget.admin_cell_changes
    ...
    async def undo_session(
        ...
        for idx, change in enumerate(changes):
            ...
            try:
                ...
                await self._log_cell_change(
                    cell_id=UUID(str(cell_id)),
                    version_id=change_dict["version_id"],
                ...
                affected_modules.add((change_dict["module_code"], str(change_dict["version_id"])))
    ...
    async def add_comment(
        ...
        query = """
            INSERT INTO efir_budget.admin_cell_comments (
    ...
    async def get_cell_comments(
        ...
        query = """
            SELECT id, cell_id, comment_text, is_resolved, created_by, created_at,
                   resolved_by, resolved_at
            FROM efir_budget.admin_cell_comments
    ...
    async def resolve_comment(
        ...
        query = """
            UPDATE efir_budget.admin_cell_comments
    ...
    async def _log_cell_change(
        ...
        query = """
            INSERT INTO efir_budget.admin_cell_changes (
                id, cell_id, version_id, module_code, entity_id, field_name,
                period_code, old_value_numeric, old_value_text, new_value_numeric, new_value_text,
                change_type, session_id, sequence_number, changed_by, changed_at
            ) VALUES (
    ...
    async def _invalidate_module_cache(
        ...
        if cache_entity:
            try:
                await CacheInvalidator.invalidate(version_id, cache_entity)
                logger.debug(
                    "cache_invalidated",
                    module_code=module_code,
