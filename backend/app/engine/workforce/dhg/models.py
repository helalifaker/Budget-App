...
class TRMDGapResult(BaseModel):
    ...
    required_fte: Decimal = Field(..., description="Required FTE based on DHG hours")
    available_aefe_fte: Decimal = Field(..., description="Available AEFE-funded FTE")
    available_local_fte: Decimal = Field(..., description="Available locally-hired FTE")
    gap_fte: Decimal = Field(..., description="Gap in FTE (positive = understaffed)")
    is_overstaffed: bool = Field(..., description="Whether position is overstaffed")
    gap_coverage_recommendation: str = Field(..., description="Recommendation for covering gap")
