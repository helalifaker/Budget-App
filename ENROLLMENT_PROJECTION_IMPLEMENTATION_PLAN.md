...
CREATE INDEX idx_projection_configs_version ON enrollment_projection_configs(budget_version_id);
...
- **Versioned tables** (`enrollment_projection_configs`, overrides, `enrollment_projections`): access is tied to the linked `budget_versions` status.
...
CREATE POLICY "enrollment_grade_overrides_manager_working"
  ...
  USING (
    ...
    AND EXISTS (
      ...
      JOIN efir_budget.budget_versions bv ON bv.id = pc.budget_version_id
...
- `backend/app/models/enrollment_projection.py`
  ...
  - [ ] Relationships to `budget_versions`, `academic_cycles`, `academic_levels`, and audit fields.
